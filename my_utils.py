...
def plot_durations(show_result=True,real_list=[],output_list=[]):
    plt.figure(1)

    duraition_real_loss = torch.tensor(real_list, dtype=torch.float)
    duraition_output_loss = torch.tensor(output_list, dtype=torch.float)
    ...
    plt.plot(duraition_real_loss.numpy(),label='real')
    plt.legend()
    plt.plot(duraition_output_loss.numpy(),label='output')
    plt.legend()
